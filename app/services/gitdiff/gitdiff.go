...
outer:
	...
	if len(filesChangedSinceLastDiff) > 0 {
		err := pull_model.UpdateReviewState(ctx, review.UserID, review.MergeRequestID, review.CommitSHA, filesChangedSinceLastDiff)
		if err != nil {
			log.Warn("Could not update review for user %d, pull %d, commit %s and the changed files %v: %v", review.UserID, review.MergeRequestID, review.CommitSHA, filesChangedSinceLastDiff, err)
			return nil, err
		}
	}
