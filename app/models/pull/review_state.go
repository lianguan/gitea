...
type ReviewState struct {
	...
	MergeRequestID       int64                  `xorm:"NOT NULL INDEX UNIQUE(pull_commit_user) DEFAULT 0"` // Which PR was the review on?
...
func GetReviewState(ctx context.Context, userID, mergeRequestID int64, commitSHA string) (*ReviewState, bool, error) {
	review := &ReviewState{UserID: userID, MergeRequestID: mergeRequestID, CommitSHA: commitSHA}
...
func UpdateReviewState(ctx context.Context, userID, mergeRequestID int64, commitSHA string, updatedFiles map[string]ViewedState) error {
	log.Trace("Updating review for user %d, repo %d, commit %s with the updated files %v.", userID, mergeRequestID, commitSHA, updatedFiles)

	review, exists, err := GetReviewState(ctx, userID, mergeRequestID, commitSHA)
	if err != nil {
		return err
	}
	...
	} else if previousReview, err := getNewestReviewStateApartFrom(ctx, userID, mergeRequestID, commitSHA); err != nil {
		return err
	...
	if !exists {
		log.Trace("Inserting new review for user %d, repo %d, commit %s with the updated files %v.", userID, mergeRequestID, commitSHA, review.UpdatedFiles)
		_, err := engine.Insert(review)
		return err
	}
	log.Trace("Updating already existing review with ID %d (user %d, repo %d, commit %s) with the updated files %v.", review.ID, userID, mergeRequestID, commitSHA, review.UpdatedFiles)
...
func GetNewestReviewState(ctx context.Context, userID, mergeRequestID int64) (*ReviewState, error) {
	var review ReviewState
	has, err := db.GetEngine(ctx).Where("user_id = ?", userID).And("merge_request_id = ?", mergeRequestID).OrderBy("updated_unix DESC").Get(&review)
	if err != nil || !has {
		return nil, err
	}
	return &review, err
...
// The returned PR Review will be nil if the user has not yet reviewed this PR.
func getNewestReviewStateApartFrom(ctx context.Context, userID, mergeRequestID int64, commitSHA string) (*ReviewState, error) {
	var reviews []ReviewState
	err := db.GetEngine(ctx).Where("user_id = ?", userID).And("merge_request_id = ?", mergeRequestID).OrderBy("updated_unix DESC").Limit(2).Find(&reviews)
